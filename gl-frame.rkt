...
(module gl-frame racket/gui
  ...
  (define glcanvas%
    (class canvas%
      ...
      (define/override (on-event event) (send (new mouse-event% [event-type 'motion] [x 100]) set-x 100)) ;(new mouse-event% [event-type 'x] [x 0])

      (super-new (style '(gl no-autoclear)))
      (set-cursor (make-object cursor% 'blank))
      (send (new mouse-event% [event-type 'motion] [x 100]) set-x 100)))
